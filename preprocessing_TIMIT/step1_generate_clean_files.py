...
TIMIT_path = "/Users/huan/Downloads/TIMIT"                          # TIMIT dataset location
target_root = "/Users/huan/Downloads/TIMIT_speakers"    # target folder for generated speech
...
for i in range(1, 9):
    ...
    # randomly select a few  male speakers from "speakers"
    ...
    # randomly select a few female speakers from "speakers"
    np.random.shuffle(speakers)
    for s in speakers[:4]:
        train_list += [entry.path for entry in os.scandir(os.path.join(r, s)) if (entry.name.endswith('.WAV') and not entry.name.startswith('SA'))]  # SA sentances are excluded
